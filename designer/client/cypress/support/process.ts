...
const createTestProcess = (name?: string, fixture?: string, category = "Category1") =>
    createProcess(name, fixture, category);

const createTestFragment = (name?: string, fixture?: string, category = "Category1") =>
    createProcess(name, fixture, category, true);
...
function getTestProcesses(filter?: string) {
    ...
    return cy
        .request({ url })
        .then(({ body }) =>
            body.filter(({ id }) => id.includes(filter || Cypress.env("processName"))).map(({ id }) => id)
        );
}
