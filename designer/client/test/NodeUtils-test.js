...
describe("edgeType retrieved", () => {
    ...
    it("should get edge types for node", () => {
        expect(
            NodeUtils.edgesForNode({ id: "node1", type: "SubprocessInput", ref: { id: "sub1" } }, processDefinitionData)
        ).toEqual({
    ...
    it("should get empty types for defaultNode", () => {
        ...
        expect(
            NodeUtils.edgesForNode({ id: "node1", type: "Processor", service: { id: "sub1" } }, processDefinitionData)
...
describe("can make link", () => {
    ...
    it("cannot make more than 2 links for Filter", () => {
        ...
        expect(
            NodeUtils.canMakeLink("filter1", "sink", createSimpleProcess(baseEdges), simpleProcessDefinition())
        ).toEqual(true);
        expect(
            NodeUtils.canMakeLink(
                "filter1",
...
const simpleProcessDefinition = () => {
    return {
        edgesForNodes: [
            {
                nodeId: { type: "Filter" },
                edges: [{ type: "FilterTrue" }, { type: "FilterFalse" }],
                canChooseNodes: false,
            },
            { nodeId: { type: "Split" }, edges: [], canChooseNodes: true },
            {
                nodeId: { type: "Switch" },
                edges: [
                    { type: "NextSwitch", condition: { language: "spel", expression: "true" } },
                    { type: "SwitchDefault" },
                ],
                canChooseNodes: true,
            },
