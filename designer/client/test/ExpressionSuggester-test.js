...
const variables = {
    ...
    dynamicMap: {
        ...
        fields: { intField: { refClazzName: "java.lang.Integer" }, aField: { refClazzName: "org.A" } },
    },
    listVar: { refClazzName: "org.WithList" },
    util: { refClazzName: "org.Util" },
    union: { union: [{ refClazzName: "org.A" }, { refClazzName: "org.B" }, { refClazzName: "org.AA" }] },
    unionOfLists: {
        union: [
...
describe("expression suggester", () => {
    ...
    it("should suggest dict variable methods", (done) => {
        ...
        suggestionsFor("#dict.", null, stubbedDictSuggestions)
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
            .then(done);
    ...
    it("should suggest in complex expression #1", (done) => {
        suggestionsFor("#input.foo + #input.barB.bazC.quax", { row: 0, column: "#input.foo".length })
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
            .then(done);
    ...
    it("should not suggest anything if suggestion already applied with space at the end", (done) => {
        suggestionsFor("#input.fooString ")
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
            .then(done);
    ...
    it("should suggest #this fields in simple projection", (done) => {
        suggestionsFor("#listVar.listField.![#this.f]", { row: 0, column: "#listVar.listField.![#this.f".length })
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
            .then(done);
    ...
    it("should suggest #this fields in projection on union of lists", (done) => {
        suggestionsFor("#unionOfLists.![#this.f]", { row: 0, column: "#unionOfLists.![#this.f".length })
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
            .then(done);
    ...
    it("should suggest #this fields in projection after selection", (done) => {
        suggestionsFor("#listVar.listField.?[#this == 'value'].![#this.f]", {
            row: 0,
            column: "#listVar.listField.?[#this == 'value'].![#this.f".length,
        })
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
    ...
    it("should support nested method invocations", (done) => {
        suggestionsFor("#util.now(#other.quaxString.toUpperCase().)", {
            row: 0,
            column: "#util.now(#other.quaxString.toUpperCase().".length,
        })
            .then((suggestions) => {
                expect(suggestions).toMatchSnapshot();
            })
...
describe("normalize multiline input", () => {
    ...
    it("normalize multiline input #1", () => {
        const extracted = expressionSuggester._normalizeMultilineInputToSingleLine(
            "#input\n  .barB.bazC\n  .quaxString.",
            { row: 1, column: "  .barB.bazC".length }
        );
    ...
    it("normalize multiline input #2", () => {
        const extracted = expressionSuggester._normalizeMultilineInputToSingleLine(
            "#input\n  .barB.bazC\n  .quaxString.",
            { row: 2, column: "  .quaxString.".length }
        );
        expect(extracted).toEqual({
            normalizedInput: "#input.barB.bazC.quaxString.",
