...
const reducer: Reducer<BackendNotificationState> = (
    state: BackendNotificationState = emptyNotifications,
    action: Action
) => {
    switch (action.type) {
        case "UPDATE_BACKEND_NOTIFICATIONS":
            return {
                processedNotificationIds: uniq(
                    state.processedNotificationIds.filter((k) => action.notificationIds.includes(k))
                ),
            };
