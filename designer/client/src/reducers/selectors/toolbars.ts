...
export const getToolbarsConfig = createSelector(
    ...
    isSubprocess,
    isArchived,
    (settings, subprocess, archived) =>
        settings?.processToolbarsConfiguration || defaultToolbarsConfig(subprocess, archived)
);
export const getToolbarsConfigId = createSelector(
    getToolbarsConfig,
    getToolbarsState,
    (c, t) => c?.id || t?.currentConfigId
);
export const getToolbars = createSelector(
    getToolbarsState,
    getToolbarsConfigId,
    (t, id) => t?.[`#${id}`] || ({} as ToolbarsState)
);
