...
class ProcessUtils {
    ...
    _findVariablesBasedOnGraph = (
        nodeId: NodeId,
        process: Process,
        processDefinition: ProcessDefinition
    ): VariableTypes => {
        const filteredGlobalVariables = pickBy(
            processDefinition.globalVariables,
            (variable) => variable.returnType !== null
        );
    ...
    findAvailableVariables =
        ...
        (nodeId: NodeId, parameterDefinition?: UIParameter): VariableTypes => {
            ...
            const variablesForNode =
                variablesFromValidation || this._findVariablesBasedOnGraph(nodeId, process, processDefinition);
            const variablesToHideForParam = parameterDefinition?.variablesToHide || [];
            const withoutVariablesToHide = pickBy(
                variablesForNode,
                (va, key) => !variablesToHideForParam.includes(key)
            );
