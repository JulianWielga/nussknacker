...
interface GenericActionDialogProps {
    ...
    setValue: (
        value: ((prevState: { [p: string]: Expression }) => { [p: string]: Expression }) | { [p: string]: Expression }
    ) => void;
}
...
function GenericActionForm(props: GenericActionDialogProps): JSX.Element {
    ...
    useEffect(() => {
        const parameterValidators = action?.parameters.reduce((object, uiParam) => {
            const paramValidators = simpleEditorValidators(
                uiParam,
                ...
                <NodeTable>
                    <NodeTableBody>
                        {(action?.parameters || []).map((param) => {
                            ...
                            const formatter =
                                param.defaultValue.language === ExpressionLang.SpEL
                                    ? spelFormatters[param?.typ?.refClazzName]
                                    : null;
                            return (
