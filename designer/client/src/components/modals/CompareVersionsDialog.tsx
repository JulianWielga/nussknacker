...
class VersionsForm extends React.Component<Props, State> {
    ...
    isLayoutChangeOnly(diffId: string): boolean {
        ...
        if (type === "NodeDifferent") {
            return this.differentPathsForObjects(currentNode, otherNode).every((path) =>
    ...
    loadVersion(versionId: string) {
        if (versionId) {
            HttpService.compareProcesses(
                ...
                this.setState({ difference: response.data, otherVersion: versionId, currentDiffId: null })
            );
        } else {
    ...
    versionDisplayString(versionId: string) {
        return this.isRemote(versionId)
            ? `${this.versionToPass(versionId)} on ${this.props.otherEnvironment}`
            : versionId;
    }
    ...
    createVersionElement(version: ProcessVersionType, versionPrefix = "") {
        ...
        return (
            <option key={versionId} value={versionId}>
                {this.versionDisplayString(versionId)} - created by {version.user} &nbsp;{" "}
                {formatAbsolutely(version.createDate)}
            </option>
        );
    }
            ...
            <>
                <div className="esp-form-row">
                    ...
                    >
                        ...
                        {this.state.remoteVersions.map((version) =>
                            this.createVersionElement(version, this.remotePrefix)
                        )}
                    </SelectWithFocus>
                </div>
