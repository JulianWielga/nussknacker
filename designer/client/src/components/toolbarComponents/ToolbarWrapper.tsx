...
const { panelBackground, panelHeaderTextSize, sidebarWidth } = styleVariables;
...
const StyledPanel = styled(Panel)(({ expanded }) => ({
    ...
    transition: `all ${expanded ? 0.3 : 0.2}s ease-in-out`,
}));
...
export function ToolbarWrapper(props: ToolbarWrapperProps): JSX.Element | null {
    ...
    return (
        <StyledPanel expanded={!isCollapsed(id)} onToggle={onToggle} bsClass={bsClass} className={themeClassName}>
            {!noTitle && (
