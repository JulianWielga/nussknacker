...
export function applyCellChanges(
    ...
    const indexed = flatMap(groupBy(edgesWithGroups, "from"), (edges) =>
        edges.map((edge, i) => ({ ...edge, index: ++i }))
    );
    const defs = Array.from(paper?.defs?.children);
    const edges = indexed
        .filter(isEdgeConnected)
        .map((value) => makeLink(value, defs.find((def) => def.nodeName === "marker")?.id));
