...
class NodeUtils {
    ...
    getEdgeById = (edgeId: NodeId, process: Process) =>
        this.edgesFromProcess(process).find((e) => this.edgeId(e) === edgeId);
    ...
    isAvailable = (node: NodeType, processDefinitionData, category): boolean => {
        const availableIdsInCategory = ProcessDefinitionUtils.getFlatCategoryComponents(
            processDefinitionData,
            category
    ...
    getIncomingEdges = (nodeId: NodeId, process: Process): Edge[] =>
        this.edgesFromProcess(process).filter((e) => e.to === nodeId);
    ...
    edgeType = (allEdges: Edge[], node: NodeType, processDefinitionData: ProcessDefinitionData): EdgeType => {
        ...
        if (edgesForNode.canChooseNodes) {
            ...
            return edgesForNode.edges.find(
                (et) => !currentConnectionsTypes.find((currentType) => _.isEqual(currentType, et))
            );
        }
    };
    ...
    canMakeLink = (
        ...
        const nodeOutputs = this._nodeOutputs(fromId, process).filter(
