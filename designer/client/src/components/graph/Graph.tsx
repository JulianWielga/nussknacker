...
export class Graph extends React.Component<Props> {
    ...
    createPaper = (): dia.Paper => {
        const canEditFrontend =
            this.props.loggedUser.canEditFrontend(this.props.processCategory) && !this.props.readonly;
    ...
    componentWillUpdate(nextProps: Props): void {
        const processChanged =
            !isEqual(this.props.processToDisplay, nextProps.processToDisplay) ||
            !isEqual(this.props.layout, nextProps.layout) ||
            !isEqual(this.props.processDefinitionData, nextProps.processDefinitionData);
        if (processChanged) {
            this.drawGraph(nextProps.processToDisplay, nextProps.layout, nextProps.processDefinitionData);
        }
    ...
    validateConnection = (
        ...
        return (
            magnetT.getAttribute("port") === "In" &&
            NodeUtils.canMakeLink(from, to, processToDisplay, processDefinitionData, previousEdge)
        );
    };
