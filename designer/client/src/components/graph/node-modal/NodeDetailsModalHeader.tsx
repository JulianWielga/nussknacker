...
const NodeClassDocs = ({ nodeClass, docsUrl }: { nodeClass?: string; docsUrl?: string }) => {
    switch (getModalHeaderType(docsUrl, nodeClass)) {
        case HeaderType.SUBTYPE_DOCS:
            return (
                <Docs docsUrl={docsUrl} className="modal-subtype-header-docs">
                    {nodeClass}
                </Docs>
            );
        ...
        case HeaderType.DOCS:
            return (
                <Docs docsUrl={docsUrl} className="modal-docs-link">
                    {nodeClass}
                </Docs>
            );
        default:
...
const NodeDetailsModalHeader = ({ node }: { node: NodeType }): JSX.Element => {
    ...
    const { docsUrl } = useMemo(
        () => componentsConfig[ProcessUtils.findNodeConfigName(node)] || {},
        [componentsConfig, node]
    );
