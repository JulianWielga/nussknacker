    ...
    return (
        <NodeTableBody className="node-test-results">
            ...
            {Object.keys(results.testResultsToShow.context.variables).map((key, ikey) => {
                const variable = results.testResultsToShow.context.variables[key];
                return (
                    <div className="node-row" key={ikey}>
                        <div className="node-label">{key}:</div>
                        <TestValue value={variable} shouldHideTestResults={false} />
                    </div>
                );
            })}
            {results.testResultsToShow && !isEmpty(results.testResultsToShow.externalInvocationResultsForCurrentContext)
                ...
                : null}
            {results.testResultsToShow &&
            !isEmpty(results.testResultsToShow.externalInvocationResultsForEveryContext) ? (
                <span className="testResultDownload">
                    <a
                        download={`${nodeId}-all-inputs.log`}
                        href={downloadableHref(
                            mergedMockedResults(results.testResultsToShow.externalInvocationResultsForEveryContext)
                        )}
                    >
