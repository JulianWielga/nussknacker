...
export const literalIntegerValueValidator: Validator = {
    ...
    description: () =>
        i18next.t("literalIntegerValueValidator.description", "Please fill field by proper integer type"),
...
export const minimalNumberValidator = (minimalNumber: number): Validator => ({
    ...
    message: () =>
        i18next.t(
            "minNumberValidator.message",
            "This field value has to be a number greater than or equal to {{min}}",
            { min: minimalNumber }
        ),
...
export const validators: Record<BackendValidator, (...args: any[]) => Validator> = {
    ...
    [BackendValidator.RegExpParameterValidator]: ({ pattern, message, description }) =>
