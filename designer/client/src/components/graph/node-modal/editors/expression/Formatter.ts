...
export const stringSpelFormatter: Formatter = {
    encode: (value) => {
        ...
        return flow([escapeQuotes(qm), templatesToConcats(qm), unescapeTemplates, quote(qm), simplifyQuotes(qm)])(
            value
        );
    },
    decode: (value) => {
...
const spelDateTimeFormatter: Formatter = {
    // eslint-disable-next-line i18next/no-literal-string
    encode: (m: moment.Moment) =>
        `T(java.time.LocalDateTime).parse('${m.startOf("minute").format("YYYY-MM-DDTHH:mm")}')`,
    decode: (value) => {
        const result = /^T\(java\.time\..*\)\.parse\(['"](.*)['"]\)$/.exec(value);
        return result == null ? null : result[1];
    },
