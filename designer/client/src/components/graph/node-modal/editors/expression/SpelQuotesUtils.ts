...
const replace = (value: string, searchValue: string, replaceValue: string) =>
    value.replaceAll ? value.replaceAll(searchValue, replaceValue) : value.split(searchValue).join(replaceValue);
...
const switchQuotes = curry((quoteBefore: QuotationMark, quoteAfter: QuotationMark, value: string): string => {
    return flow([unquote(quoteBefore), unescapeQuotes(quoteBefore), escapeQuotes(quoteAfter), quote(quoteAfter)])(
        value
    );
});
