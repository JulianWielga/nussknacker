...
export function ComponentPreview({
    ...
    const nodeStyles = css({
        ...
        transform: `translate(-80%, -50%) scale(${isOver ? 1 : 0.9}) rotate(${
            isActive ? (isOver ? -2 : 2) : 0
        }deg) scale(${isActive ? 1 : 1.5})`,
        ...
        <div className={cx(colors, nodeStyles)}>
            ...
            <div className={contentStyles}>
                ...
                {NodeUtils.hasInputs(node) && (
                    <Port className={cx(css({ top: 0, transform: "translateY(-50%)" }), colors)} />
                )}
                {NodeUtils.hasOutputs(node) && (
                    <Port className={cx(css({ bottom: 0, transform: "translateY(50%)" }), colors)} />
                )}
            </div>
        </div>
    );
