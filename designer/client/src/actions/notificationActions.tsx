...
export function success(message: string): Action {
    return Notifications.success({
        autoDismiss: 10,
        children: [<Notification icon={<TipsSuccess />} message={message} key={uuid4()} />],
    });
}
...
export function error(message: string, error?: string, showErrorText?: boolean): Action {
    ...
    return Notifications.error({
        autoDismiss: 10,
        children: [<Notification icon={<TipsError />} message={message} details={details} key={uuid4()} />],
    });
}
...
export function info(message: string): Action {
    return Notifications.info({
        autoDismiss: 10,
        children: [<Notification icon={<TipsInfo />} message={message} key={uuid4()} />],
    });
}
