...
function alignSubprocessesNodeWithSchema(process, processDefinitionData) {
    return {
        ...
        nodes: process.nodes.map((node) => {
            return node.type === "SubprocessInput" ? alignSubprocessWithSchema(processDefinitionData, node) : node;
        }),
    };
}
...
export function calculateProcessAfterChange(
    ...
    return async (dispatch, getState) => {
        if (NodeUtils.nodeIsProperties(after)) {
            const processDef = await dispatch(
                fetchProcessDefinition(process.processingType, process.properties.isSubprocess)
            );
            const processWithNewSubprocessSchema = alignSubprocessesNodeWithSchema(
                process,
